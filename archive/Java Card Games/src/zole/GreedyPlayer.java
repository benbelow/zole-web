...
public class GreedyPlayer extends Player implements PlayerInterface {

	public ArrayList<Card> toscore = new ArrayList<Card>();

	public GreedyPlayer(int position, String name) {
		super(position, name);
		playertype = "AI";
	}
					...
					if (spadenumber == 1 && clubnumber == 1 || spadenumber == 1
						...
						if ((spadenumber == 1 || heartnumber == 1 || clubnumber == 1)
								|| (spadenumber == 2 && clubnumber == 2
										...
										&& clubnumber == 2)) {
							System.out
									.println("3: Eliminates one of two suits");
							...
							scoretoscore();
									...
									// can be eliminated
							System.out
									.println("4: Eliminate only suit possible with 2");
							for (Card card : hand.cards_in_hand) {
							...
							|| heartnumber == 1) {
						System.out
								.println("5: Eliminate one suit + score highly");
						for (Card card : hand.cards_in_hand) {
							if (hand.number_of_suit(card.zolesuit) == 1
									&& putdown < 2) {
								toscore.add(card);
	...
	public void scoretoscore() { // to avoid concurrentmodificationexception.
		...
		while (!toscore.isEmpty()) {
			System.out.println("Scoring " + toscore.get(0).value + " of "
					+ toscore.get(0).suit);
